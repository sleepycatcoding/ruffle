...
pub fn append_child<'gc>(
    ...
    let children = xml
        .get_property_local(&Multiname::any(activation.gc()), activation)?
        .as_object()
        .and_then(|x| x.as_xml_list_object())
        .expect("Should be XMLList");
    children.set_property_local(
        &Multiname::new(
            activation.avm2().public_namespace,
            AvmString::new_utf8(activation.gc(), children.length().to_string()),
        ),
        child,
        activation,
    )?;
